...
function SnapshotCard({
  ...
  return (
    <div className="min-w-[280px] bg-slate-950 p-4 rounded-lg border border-slate-700 relative flex flex-col">
      <button
        onClick={() => removeSnapshot(snapshot.id)}
        className="absolute top-2 right-2 w-6 h-6 bg-slate-800 text-slate-400 rounded hover:bg-red-500 hover:text-white transition-colors flex items-center justify-center text-xs font-bold"
        ...
        <div className="min-w-0 flex-1">
          {editing ? (
            <input
              ...
              className="w-full bg-slate-800 text-slate-100 text-sm font-bold px-2 py-1 rounded border border-amber-500 focus:outline-none"
            />
          ) : (
            <button
      ...
      {/* TTK badge */}
      {snapshot.ttk !== null && (
        <div className="mb-3 px-3 py-1.5 bg-slate-900 rounded border border-slate-700 flex items-center justify-between">
...
export const ComparisonGrid: React.FC = () => {
  ...
  if (snapshots.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center h-64 text-slate-500 bg-slate-900/50 rounded-xl border border-dashed border-slate-800">
