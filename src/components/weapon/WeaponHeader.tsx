...
/** Card / icon border colour driven by variant and rarity. */
...
export const WeaponHeader: React.FC = () => {
  const { activeWeapon, variantGroup, loadWeapon } = useWeaponStore(
    useShallow((s) => ({
  ...
  return (
    <div className={`relative rounded-xl overflow-hidden border min-h-[280px] md:min-h-[320px] ${cardBorderClass(activeWeapon)}`}>
      ...
      <div className="relative z-10 p-4 md:p-6 flex flex-col gap-3">
        <div className="max-w-sm space-y-2.5">
          ...
          <div className="flex items-center gap-3">
            <div className={`relative w-14 h-14 rounded-lg overflow-hidden border-2 shrink-0 bg-white/5 transition-colors duration-300 ${cardBorderClass(activeWeapon)}`}>
              <Image
                ...
                return (
                  ...
                  >
                    <span className={['relative inline-flex w-7 h-4 rounded-full transition-colors border', isAdeptSelected ? 'bg-amber-500/40 border-amber-500/60' : 'bg-white/5 border-white/15'].join(' ')}>
                      <span className={['absolute top-0.5 w-3 h-3 rounded-full transition-transform', isAdeptSelected ? 'translate-x-3.5 bg-amber-400' : 'translate-x-0.5 bg-slate-500'].join(' ')} />
                    </span>
