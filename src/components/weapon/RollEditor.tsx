  ...
  return (
    <div className="bg-slate-900/50 backdrop-blur-sm p-4 md:p-6 rounded-xl border border-slate-800">
            ...
            {column.perks.map((perk) => {
              ...
              return (
                <button
                  ...
                  title={`${perk.name}${perk.isEnhanced ? ' (Enhanced)' : ''}: ${perk.description}`}
                  aria-pressed={isActive}
                  className={
                    'relative w-12 h-12 md:w-14 md:h-14 rounded-full border-2 transition-all duration-200 overflow-hidden ' +
                    (isActive
                      ? 'border-amber-400 shadow-[0_0_15px_rgba(251,191,36,0.4)] scale-110'
      ...
      <p className="text-xs text-slate-600 mt-4">
        Click a selected perk to deselect. <span className="text-green-500">●</span> = auto-activates buff.{' '}
