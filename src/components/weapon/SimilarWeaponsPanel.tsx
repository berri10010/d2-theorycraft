...
/** Sum of all base stats — a rough "total stat budget" proxy. */
function totalStatBudget(w: Weapon): number {
  return Object.values(w.baseStats).reduce((sum: number, v: number) => sum + v, 0);
}
...
/** Compute a recommendation score vs the active weapon.
...
function similarityScore(
  ...
  // Must share the same intrinsic archetype frame
  if (archetypeOf(candidate) !== archetypeOf(active)) return null;
  // Must be same weapon type (Hand Cannon, Auto Rifle, etc.)
  if (candidate.itemTypeDisplayName !== active.itemTypeDisplayName) return null;

  const delta = Math.abs(totalStatBudget(candidate) - activeTotal);
  return delta;
}
...
function SimilarRow({
  ...
  const budget = totalStatBudget(weapon);
  const delta = budget - activeTotal;
  const deltaStr =
    delta === 0
      ? '='
      : delta > 0
  ...
  // Element colour from damageType
  ...
  return (
    <div className="flex items-center gap-3 py-2 border-b border-white/5 last:border-0">
      ...
      {/* Stat budget delta */}
      <span className={`text-[10px] font-bold tabular-nums shrink-0 ${deltaClass}`}>
        {deltaStr}
      </span>
...
export const SimilarWeaponsPanel: React.FC = () => {
  ...
  const recommendations = useMemo<{ weapon: Weapon; score: number }[]>(() => {
    ...
    const activeTotal = totalStatBudget(activeWeapon);
    // Collect all weapon hashes in the active variant group so we skip them
    ...
    for (const w of weapons) {
      ...
      const score = similarityScore(activeWeapon, w, activeTotal);
      if (score !== null) scored.push({ weapon: w, score });
    }

    // Sort by similarity (ascending delta), then newest season first as tiebreak
    scored.sort((a, b) => {
  ...
  return (
    <div className="bg-white/5 backdrop-blur-sm p-4 md:p-6 rounded-xl border border-white/10">
      ...
      <p className="text-[11px] text-slate-500 mb-4">
        Same archetype · ranked by closest total stat budget
      </p>
      ...
      <div className="flex items-center gap-3 pb-1.5 mb-1 border-b border-white/10">
        ...
        <span className="text-[9px] text-slate-600 uppercase tracking-wider shrink-0">Budget Δ</span>
